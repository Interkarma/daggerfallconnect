...
namespace XNALibrary
{
    ...
    public class Sky : Component
    {
            ...
            : base(graphicsDevice)
        {
            ...
            skyVertexDeclaration = new VertexDeclaration(
                VertexPositionNormalTexture.VertexDeclaration.GetVertexElements());

            // Create sky effect
            skyEffect = new BasicEffect(graphicsDevice);
        ...
        private void UpdateTextures()
        {
            ...
            if (skyFrame < 32)
            {
                ...
                west = skyFile.GetBitmapFormat(0, skyFrame, 0, DFBitmap.Formats.RGBA);
                east = skyFile.GetBitmapFormat(1, skyFrame, 0, DFBitmap.Formats.RGBA);
            }
            else
            {
                skyFile.Palette = skyFile.GetDFPalette(63 - skyFrame);
                east = skyFile.GetBitmapFormat(0, 63 - skyFrame, 0, DFBitmap.Formats.RGBA);
                west = skyFile.GetBitmapFormat(1, 63 - skyFrame, 0, DFBitmap.Formats.RGBA);
            }

            // Create textures
            westTexture = new Texture2D(GraphicsDevice, west.Width, west.Height, false, SurfaceFormat.Color);
            eastTexture = new Texture2D(GraphicsDevice, east.Width, east.Height, false, SurfaceFormat.Color);

            // Set data
            westTexture.SetData<byte>(0, null, west.Data, 0, west.Width * west.Height * 4);
            eastTexture.SetData<byte>(0, null, east.Data, 0, west.Width * west.Height * 4);
