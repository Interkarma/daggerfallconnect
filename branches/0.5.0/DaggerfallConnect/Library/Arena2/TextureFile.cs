...
namespace DaggerfallConnect.Arena2
{
    ...
    public class TextureFile : DFImageFile
    {
        ...
        /// <summary>
        /// Get raw data as A8R8G8B8 for specified record and frame. Colour values will be copied into adjacent alpha values
        ///  to prevent a border when sprites are alpha blended.
        /// </summary>
        /// <param name="Record">Index of record.</param>
        /// <param name="Frame">Index of frame.</param>
        /// <param name="AlphaIndex">Index of alpha colour.</param>
        /// <returns>DFBitmap object.</returns>
        //public DFBitmap GetDilatedBitmapA8R8G8B8(int Record, int Frame, byte AlphaIndex)
        //{
        //    // Validate
        //    if (Record < 0 || Record >= Header.RecordCount || Records == null || Frame >= GetFrameCount(Record))
        //        return new DFBitmap();
        ...
        //    // Create new raw bitmap
        //    const int padding = 2;
        //    DFBitmap dfBitmap = new DFBitmap();
        //    dfBitmap.Format = DFBitmap.Formats.A8R8G8B8;
        //    dfBitmap.Width = Records[Record].Width + padding;
        //    dfBitmap.Height = Records[Record].Height + padding;
        //    dfBitmap.Stride = dfBitmap.Width * 4;
        //    dfBitmap.Data = new byte[dfBitmap.Stride * dfBitmap.Height];

        //    // Copy bitmap with offset
        //    int offset = dfBitmap.Stride * padding / 2;
        //    ConvertIndexedToA8R8G8B8(ref Records[Record].Frames[Frame].Data, ref dfBitmap.Data, Records[Record].Width, Records[Record].Height, dfBitmap.Stride, offset - dfBitmap.Stride, AlphaIndex, true);
        //    ConvertIndexedToA8R8G8B8(ref Records[Record].Frames[Frame].Data, ref dfBitmap.Data, Records[Record].Width, Records[Record].Height, dfBitmap.Stride, offset + dfBitmap.Stride, AlphaIndex, true);
        //    ConvertIndexedToA8R8G8B8(ref Records[Record].Frames[Frame].Data, ref dfBitmap.Data, Records[Record].Width, Records[Record].Height, dfBitmap.Stride, offset - 4, AlphaIndex, true);
        //    ConvertIndexedToA8R8G8B8(ref Records[Record].Frames[Frame].Data, ref dfBitmap.Data, Records[Record].Width, Records[Record].Height, dfBitmap.Stride, offset + 4, AlphaIndex, true);
        //    ConvertIndexedToA8R8G8B8(ref Records[Record].Frames[Frame].Data, ref dfBitmap.Data, Records[Record].Width, Records[Record].Height, dfBitmap.Stride, offset, AlphaIndex, false);
        ...
        /// <summary>
        /// Converts an indexed buffer into a A8R8G8B8 buffer.
        /// </summary>
        /// <param name="SrcData">Source buffer.</param>
        /// <param name="DstData">Destination buffer.</param>
        /// <param name="SrcWidth">Source buffer width.</param>
        /// <param name="SrcHeight">Source buffer height.</param>
        /// <param name="DstStride">Destination buffer stride (pixels per row).</param>
        /// <param name="RowOffset">Offset applied to each pixel drawn.</param>
        /// <param name="AlphaIndex">The index to be treated as alpha.</param>
        /// <param name="AlphaOnly">Draw all pixels in alpha channel only.</param>
        //private void ConvertIndexedToA8R8G8B8(ref byte[] SrcData, ref byte[] DstData, int SrcWidth, int SrcHeight, int DstStride, int RowOffset, int AlphaIndex, bool AlphaOnly)
        //{
        ...
        //        int dstPos = y * DstStride + RowOffset;

        //        for (int x = 0; x < SrcWidth; x++)
        //        {
        //            // Determine alpha value for colour pixel
        //            if (AlphaOnly) a = 0x40; else a = 0xff;

        //            // Colour pixel
        //            byte index = SrcData[srcPos++];
        //            if (index != AlphaIndex)
        //            {
